...
export DATABASE_URL="${DATABASE_URL:-postgresql://postgres:ci_password@localhost:5432/inventory_db}"
...
python_runner -m flask db upgrade
python_runner -m pytest tests/data/ -v --tb=short
compose_ci down -v
...
until compose_ci exec -T db pg_isready -U postgres -d inventory_db >/dev/null 2>&1; do sleep 1; done
python_runner -m flask db upgrade
python_runner -m pytest tests/ -m "not e2e" --cov=app --cov-report=term-missing --tb=short
compose_ci down -v
