...
class TestProductCRUD:
    ...
    def test_create_product_duplicate_sku(self, app_client):
        ...
        app_client.post('/products', data=json.dumps(payload1), content_type='application/json')
        response = app_client.post(
            '/products',
    ...
    def test_create_and_get_product(self, app_client):
        ...
        get_response = app_client.get(f'/products/{product_id}')
    ...
    def test_get_nonexistent_product(self, app_client):
        ...
        response = app_client.get('/products/9999')
    ...
    def test_delete_product_success(self, app_client):
        ...
        # Eliminar
        response = app_client.delete(f'/products/{product_id}')
        assert response.status_code == 200

        # Verificar que fue eliminado
        response = app_client.get(f'/products/{product_id}')
        assert response.status_code == 404

    def test_delete_nonexistent_product(self, app_client):
        """Test: eliminar un producto que no existe"""
        response = app_client.delete('/products/9999')
        assert response.status_code == 404
...
class TestProductSearch:
    ...
    def test_search_by_name(self, app_client):
        ...
        app_client.post('/products', data=json.dumps(payload1), content_type='application/json')
        app_client.post('/products', data=json.dumps(payload2), content_type='application/json')

        response = app_client.get('/products?search=Laptop')
        assert response.status_code == 200
        data = json.loads(response.data)
        assert data['total'] >= 1
    ...
    def test_filter_by_category(self, app_client):
        ...
        response = app_client.get('/products?category=Electronics')
        assert response.status_code == 200
    ...
    def test_pagination(self, app_client):
        ...
        for i in range(15):
            ...
            app_client.post('/products', data=json.dumps(payload), content_type='application/json')

        response = app_client.get('/products?page=1&per_page=10')
        assert response.status_code == 200
        data = json.loads(response.data)
        assert len(data['products']) == 10

        response = app_client.get('/products?page=2&per_page=10')
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_sort_by_price(self, app_client):
        ...
        response = app_client.get('/products?sort_by=price&sort_order=asc')
        assert response.status_code == 200
        data = json.loads(response.data)
